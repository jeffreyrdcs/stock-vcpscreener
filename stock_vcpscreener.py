...
from stock_vcpscreener.vcp_util.util import (
    gen_report_front_page,
    gen_report_output_page,
    gen_report_combine,
...
class StockVCPScreener:
    ...
    def __init__(self, in_sel_date: date, in_stock_list: list):
        ...
        self.report_dict = self._get_default_report_dict(in_sel_date)

        # A special list of Trust or ETF
        self.special_index_stock_list = {
            "VOO": "brasil",
            "QQQ": "blueskies",
            "DIA": "mike",
            "IWM": "classic",
        ...
        self.selected_stock_list = pd.DataFrame(
            columns=[
                "Stock",
                "Index",
                "RS Rating",
        ...
        self.start_date = self._date_to_study - timedelta(days=365)
        self.end_date = self._date_to_study
    ...
    def select_stock(self, overwrite=False, writecsv=False):
        ...
        outdaily_name = self._date_to_study
        ...
        if self._date_to_study.isoweekday() in set((6, 7)):
            ...
            for ind, stock in enumerate(self._stock_list):
                ...
                if len(df.index) <= 245:
                    ...
                    if (
                        ...
                        self.selected_stock_list = self.selected_stock_list.append(
                            {
                                "Stock": stock,
                                "Index": ind,
                                "RS Rating": rs_rating,
    ...
    def generate_report(self):
        ...
        for index, cols in self.selected_stock_rs_rank_list.iterrows():
            try:
                ...
                if (RS_rank > _RANK_CRITERIA) or (name in self.special_index_stock_list.keys()):
                ...
                if name in self.special_index_stock_list.keys():
                    ...
                    mpf.plot(
                        hist,
                        **kwargs,
                        style=self.special_index_stock_list[name],
                        title=titlename,
        ...
        out_status = gen_report_breadth_page(self.report_dict, self._date_to_study, self.cdir_path)
        out_status = gen_report_combine(self.cdir_path, self.output_path, self._date_to_study)
